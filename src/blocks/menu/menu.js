...
class Menu {
  constructor({ control, items, menu }, overlayObject) {
  ...
  open() {
    ...
    if (this.menuMobile)
      this.menuControl.classList.add("menu__mobile_close_black");
    this.overlay.show();
    this.menu.classList.add(
      !this.menuMobile ? "menu_on-top" : "menu_on-top_black"
    );
    this.menuItems.classList.add("menu__items-list_show");
    if (this.menuMobile)
      this.menuItems.style.background = "rgba(255,255,255,1)";
  ...
  close() {
    ...
    if (this.menuMobile)
      this.menuItems.style.background = "rgba(255,255,255,0)";
    this.menuOpened = false;
  }
}
